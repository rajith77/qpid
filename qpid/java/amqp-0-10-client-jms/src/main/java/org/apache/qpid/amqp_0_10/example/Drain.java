...
public class Drain extends OptionParser
{
    ...
    static final Option FOREVER = new Option("f", "forever", "ignore timeout and wait forever", null, null,
            Boolean.class);

    static final Option COUNT = new Option("c", "count", "read c messages, then exit", "COUNT", "0", Integer.class);
    ...
    public Drain(String[] args, String usage, String desc) throws Exception
    {
        ...
        if (containsOp(TIMEOUT))
        {
            timeout = Integer.parseInt(getOp(TIMEOUT)) * 1000;
        }
        if (containsOp(FOREVER))
        {
            timeout = 0;
        }
        if (containsOp(COUNT))
        {
            count = Integer.parseInt(getOp(COUNT));
        }
